...
# x => "2019-01-11 오후 05:32" -> 2019-01-11 05:32
def convert_time(x):    # x : 하나 하나의 값(str)
    date, ampm, time = x.split()
    if ampm == "오후" and time != "12:00":   # 오후이면서 12시 전이면
        hour, minute = map(int, time.split(":"))
        time = f"{hour+12 if hour != 12 else hour}:{minute}"
    return pd.to_datetime(f"{date} {time}", format="%Y-%m-%d %H:%M")
    
pass
# apply : function, 변경할data값(또는 리스트값)
## series (x : 1개일때)
...
def receive_params(params):
    ...
    # '2019-01-11 오후 05:32'
    return params
df_datetimes[['완료일_한글', '신청일']].apply(receive_params, axis=1)   # axis : 축
pass